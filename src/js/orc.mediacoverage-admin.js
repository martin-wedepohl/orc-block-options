(function ($, wp) {
    ...
    customInlineEditPost.edit = function (id) {
        ...
        // Custom meta.
        if ($(':input[name="featured_on"]', editRow).length === 1) {
            var featuredOn = $('#post-' + id + ' .featured_on').text();
            $(':input[name="featured_on"]', editRow).val(featuredOn);
        }
        if ($(':input[name="featured_in"]', editRow).length === 1) {
            var featuredIn = $('#post-' + id + ' .featured_in').text();
            $(':input[name="featured_in"]', editRow).val(featuredIn);
        }
